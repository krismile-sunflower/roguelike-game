...
func set_state(new_state: State) -> void:
    ...
    match current_state:
        ...
        State.PAUSED:
            ...
            # 暂停背景音乐
            if AudioManager:
            (AudioManager as Node).call_deferred("toggle_bgm_pause")
        ...
        State.GAME_OVER:
            ...
            # 保存最高分到存档
            if SaveManager:
                (SaveManager as Node).call_deferred("save_high_score", high_score)
            # 保存游玩统计
            if SaveManager:
                (SaveManager as Node).call_deferred("increment_stats", level_cleared)
                (SaveManager as Node).call_deferred("save_play_time", get_play_time())
            # 发出游戏结束信号
            game_over.emit(GameData.score)
            # 停止背景音乐
            if AudioManager:
                (AudioManager as Node).call_deferred("stop_bgm")
...
func start_new_game() -> void:
    ...
    # 播放背景音乐
    if AudioManager:
        (AudioManager as Node).call_deferred("play_bgm", "gameplay")
...
func advance_to_next_level() -> void:
    ...
    # 保存关卡进度
    if SaveManager:
        (SaveManager as Node).call_deferred("save_level_progress", current_level)
...
func _input(event: InputEvent) -> void:
    ...
    if event.is_action_pressed("ui_escape"):
        if current_state == State.PLAYING or current_state == State.PAUSED:
            toggle_pause()

    # Enter 键重新开始（仅在游戏结束时）
    if event.is_action_pressed("ui_select") and current_state == State.GAME_OVER:
        start_new_game()

    # F5 强制保存（调试用）
    if event.is_action_pressed("ui_cancel"):  # 用 cancel 键
        if SaveManager:
            (SaveManager as Node).call_deferred("save_all")
            print("存档已手动保存")
